...
(function (factory) {
    ...
    function refreshDropMenu($input, data, options) {
        ...
        if (options.showHeader) {
            html.push('<thead><tr>');
            for (var i=0;i<options.effectiveFields.length;i++) {
				var fieldName = options.effectiveFields[i];
        ...
        for (i = 0; i < len; i++) {
            ...
            for (var j=0;j<options.effectiveFields.length;j++) {
				var fieldName = options.effectiveFields[j];
